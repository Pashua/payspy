...
* compilation of [SCSS](http://sass-lang.org/) files
...
* Must have [Ruby](http://www.ruby-lang.org/) installed.
* Must have [Compass](http://compass-style.org/) installed with Gem. `gem install compass`
* You can use [Gem Bundler](http://gembundler.com/) which will verify all dependencies. `gem install bundler` and `bundle install`
...
## Making Changes
* `grunt dev` will watch for any CoffeeScript (.coffee), Compass SCSS (.scss) or .template file changes.  When changes are detected, the files will be linted, compiled, and ready for you to refresh the browser.
